...
/*   Updated: 2017/02/10 16:37:25 by vomnes           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int ft_dir_one(t_data *data)
{
  ...
  if (X_ONE > X_ZERO && Y_ONE > Y_ZERO) //Down right
  {
      ft_select_dir(data, RIGHT_DOWN);
      return (1);
  }
  if (X_ONE == X_ZERO && Y_ONE < Y_ZERO) // Up
  {
      if (data->p_pos.x == -1)
        ft_select_dir(data, RIGHT_UP);
      else
...
{
  if (X_ONE > X_ZERO && Y_ONE == Y_ZERO) // Right
  {
      if (data->p_pos.y == -1)
        ft_select_dir(data, RIGHT_DOWN);
      else
  ...
  if (X_ONE == X_ZERO && Y_ONE > Y_ZERO) // Down
  {
      if (data->p_pos.x == -1)
        ft_select_dir(data, RIGHT_DOWN);
      else
...
{
  if (X_ONE < X_ZERO && Y_ONE > Y_ZERO) //Down left
  {
      ft_select_dir(data, LEFT_DOWN);
      return (1);
  }
  if (X_ONE < X_ZERO && Y_ONE == Y_ZERO) // Left
  {
      if (data->p_pos.y == -1)
          ft_select_dir(data, LEFT_DOWN);
      else
  ...
  if (X_ONE < X_ZERO && Y_ONE < Y_ZERO)//Up left
  {
      ft_select_dir(data, LEFT_UP);
      return (1);
  }
